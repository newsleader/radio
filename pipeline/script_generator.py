...
170~210 어절을 목표로 하되, 기사에 있는 내용(배경·원인·수치·인용·전망)만 담으세요.
기사에 없는 사실은 추가하지 마세요.
...
def generate_script(article: Article, is_breaking: bool = False) -> Optional[tuple]:
    ...
    min_words = config.SCRIPT_MIN_WORDS_BREAKING if is_breaking else config.SCRIPT_MIN_WORDS
